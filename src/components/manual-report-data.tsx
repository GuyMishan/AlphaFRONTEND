...
export function ManualReportData({ organizationId, employerId, reportId, month, employees, selectedIds, setSelectedIds }: Props) {
  ...
  const filteredEmployees = useMemo(() => {
    ...
    return employees.filter((employee) => `${employee.firstName} ${employee.lastName} ${employee.nationalId} ${employee.employeeNumber}`.toLowerCase().includes(term));
  }, [employees, query]);
  ...
  async function editEmployee(row: ManualReportEmployeeSummary) {
    setError("");
    try {
      const detail = await alphaApi.manualReportEmployee(organizationId, employerId, reportId, row.id);
      setEditing(detail);
    } catch (err) {
      setError(err instanceof Error ? err.message : "טעינת פרטי העובד נכשלה");
    }
  }
  ...
  return <>
    <div className="card-head manual-report-head">
      <div><h2>עובדים בדיווח</h2><span style={{ color: "var(--muted)" }}>בחרו את העובדים ולחצו עריכה כדי להזין מוצרים והפקדות.</span></div>
    ...
    <div className="manual-employee-list">
      {loading ? <div className="empty">טוען את עובדי הדיווח...</div> : filteredEmployees.map((employee) => {
...
function EmployeeProductsModal({ employee, month, onClose, onSave }: { employee: ManualReportEmployeeDetail; month: string; onClose: () => void; onSave: (products: ManualProductInput[]) => Promise<void> }) {
  ...
  async function save() {
    ...
    catch (err) { setError(err instanceof Error ? err.message : "שמירת נתוני העובד נכשלה"); setSaving(false); }
  }
  ...
  return <div className="report-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <div className="report-modal" role="dialog" aria-modal="true" aria-label={`עריכת ${employee.firstName} ${employee.lastName}`}>
      <div className="report-modal-header"><div><h2>{employee.firstName} {employee.lastName}</h2><span>ת״ז {employee.nationalId} · מספר עובד {employee.employeeNumber}</span></div><button className="icon-button" onClick={onClose} aria-label="סגירה"><X size={20} /></button></div>
      <div className="report-modal-body">
        {error ? <div className="notice notice-error">{error}</div> : null}
        <div className="employee-report-summary"><div><span>חודש דיווח</span><b>{month}</b></div><div><span>מספר מוצרים</span><b>{products.length}</b></div><div><span>סה״כ הפקדות</span><b>₪{products.reduce((sum, product) => sum + [...product.employerContributions, ...product.employeeContributions].reduce((s, c) => s + Number(c.amount || 0), 0), 0).toLocaleString("he-IL")}</b></div></div>
        <div className="product-editor-list">
          {products.map((product, index) => <ProductEditor key={index} index={index} product={product} updateProduct={updateProduct} updateContribution={updateContribution} remove={() => setProducts((current) => current.filter((_, i) => i !== index))} />)}
          {!products.length ? <div className="empty"><div>עדיין לא הוגדרו מוצרים לעובד בדיווח הזה.</div></div> : null}
        </div>
        <button className="btn btn-soft wide" onClick={() => setProducts((current) => [...current, emptyProduct(month)])}><Plus size={17} />הוספת מוצר</button>
      </div>
      <div className="report-modal-footer"><button className="btn btn-secondary" onClick={onClose}>ביטול</button><button className="btn btn-primary" disabled={saving} onClick={() => void save()}><Save size={17} />{saving ? "שומר..." : "שמירת נתוני העובד"}</button></div>
    </div>
  </div>;
}
...
function ProductEditor({ index, product, updateProduct, updateContribution, remove }: {
  ...
  return <section className="report-product-card">
    <div className="report-product-title"><div><span>מוצר {index + 1}</span><b>{productTypes.find((x) => x.value === product.productType)?.label ?? "מוצר פנסיוני"}</b></div><button className="icon-button danger" onClick={remove} aria-label="מחיקת מוצר"><Trash2 size={18} /></button></div>
    <div className="grid report-product-fields">
      ...
      <div className="field"><label>מספר פוליסה</label><input value={product.policyNumber} onChange={(e) => updateProduct(index, { policyNumber: e.target.value })} /></div>
      <div className="field"><label>חודש שכר</label><input type="month" value={product.salaryMonth.slice(0, 7)} onChange={(e) => updateProduct(index, { salaryMonth: `${e.target.value}-01` })} /></div>
      <div className="field"><label>שכר</label><input type="number" min="0" step="0.01" value={product.salary || ""} onChange={(e) => updateProduct(index, { salary: Number(e.target.value) })} /></div>
      ...
      <div className="field"><label>תאריך תחילת סעיף 14</label><input type="date" disabled={!product.section14} value={product.section14StartDate ?? ""} onChange={(e) => updateProduct(index, { section14StartDate: e.target.value || null })} /></div>
    </div>
    <ContributionTable title="הפקדות מעסיק" items={product.employerContributions} onChange={(component, key, value) => updateContribution(index, "employerContributions", component, key, value)} />
    <ContributionTable title="הפקדות עובד" items={product.employeeContributions} onChange={(component, key, value) => updateContribution(index, "employeeContributions", component, key, value)} />
  </section>;
}
