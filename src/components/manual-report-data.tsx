...
export function ManualReportData({ organizationId, employerId, reportId, month, employees, selectedIds, setSelectedIds }: Props) {
  ...
  async function selectAllVisibleActive() {
    ...
    setSyncing(true); setError("");
    try { await alphaApi.syncManualReportEmployees(organizationId, employerId, reportId, next); await loadRows(query.trim()); }
    catch (err) { setError(err instanceof Error ? err.message : "עדכון העובדים בדיווח נכשל"); }
    finally { setSyncing(false); }
  }
  ...
  return <>
    ...
    {editing ? <EmployeeProductsModal employee={editing} month={month} onClose={() => setEditing(null)} onSave={async (products) => { await alphaApi.saveManualReportEmployee(organizationId, employerId, reportId, editing.id, products); setEditing(null); await loadRows(query.trim()); }} /> : null}
  </>;
}
...
function EmployeeProductsModal({ employee, month, onClose, onSave }: { employee: ManualReportEmployeeDetail; month: string; onClose: () => void; onSave: (products: ManualProductInput[]) => Promise<void> }) {
  ...
  function updateProduct(index: number, patch: Partial<ManualProductInput>) { setProducts((current) => current.map((product, i) => i === index ? { ...product, ...patch } : product)); setError(""); }
  function updateContribution(productIndex: number, party: "employerContributions" | "employeeContributions", component: ContributionComponent, key: "amount" | "percentage" | "exemptPayments", value: number) { setProducts((current) => current.map((product, index) => index !== productIndex ? product : { ...product, [party]: product[party].map((item) => item.component === component ? { ...item, [key]: Number.isFinite(value) ? value : 0 } : item) })); setError(""); }
  async function save() {
