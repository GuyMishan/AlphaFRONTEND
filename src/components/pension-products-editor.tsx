...
export function PensionProductsEditor({ context, month, monthlySalary, products, editable = true, onMonthlySalaryChange, onProductsChange }: {
  ...
  return <>
    <div className="grid two-cols" style={{ marginBottom: 12 }}>
      <div className="field"><label>{context === "report" ? "שכר חודשי של העובד בדיווח *" : "שכר חודשי של העובד *"}</label><input disabled={!editable} type="number" min="0" max="10000000" step="0.01" value={monthlySalary || ""} onChange={(e) => onMonthlySalaryChange(Number(e.target.value))} /></div>
      <div className="field"><label>סה״כ שכר מבוטח מחושב</label><input disabled value={`₪${Array.from(allocation.resolved.values()).reduce((sum, value) => sum + value, 0).toLocaleString("he-IL")}`} /></div>
    </div>
    ...
    <div className="product-editor-list">{products.map((product, index) => {
      ...
      return <section className="report-product-card" key={index} style={{ opacity: product.isActive === false ? .72 : 1 }}>
        ...
        <div className="grid report-product-fields">
          ...
          {context === "report" ? <div className="field"><label>חודש שכר *</label><input disabled={!editable} required type="month" value={(product.salaryMonth ?? month ?? "").slice(0, 7)} onChange={(e) => updateProduct(index, { salaryMonth: `${e.target.value}-01` })} /></div> : null}
          ...
          <div className="field"><label>תאריך תחולה/ביטול סעיף 14{section14DateRequired ? " *" : ""}</label><input disabled={!editable || !section14DateRequired} required={section14DateRequired} type="date" value={section14DateRequired ? product.section14StartDate ?? "" : ""} onChange={(e) => updateProduct(index, { section14StartDate: e.target.value || null })} /></div>
        </div>
