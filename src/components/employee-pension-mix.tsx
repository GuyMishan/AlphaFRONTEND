...
export function EmployeePensionMix({ organizationId, employerId, employeeId, editable }: { organizationId: string; employerId: string; employeeId: string; editable: boolean }) {
  ...
  async function save() {
    if (!editable) return;
    if (products.some((product) => !product.policyNumber.trim())) { setError("יש להזין מספר פוליסה לכל מוצר."); return; }
  ...
  return <div className="card">
    <div className="card-head"><div><h2>תמהיל העובד</h2><span style={{ color: "var(--muted)" }}>המוצרים, השכר ואחוזי ההפקדה כאן משמשים כברירת מחדל לדיווחים חדשים.</span></div>{editable ? <button className="btn btn-primary" disabled={saving} onClick={() => void save()}><Save size={17} />{saving ? "שומר..." : "שמירת תמהיל"}</button> : null}</div>
    ...
    <div className="product-editor-list">
      {products.map((product, index) => <section className="report-product-card" key={index}>
        <div className="report-product-title"><div><span>מוצר {index + 1}</span><b>{productTypes.find((x) => x.value === product.productType)?.label}</b></div>{editable ? <button className="icon-button danger" onClick={() => setProducts((current) => current.filter((_, i) => i !== index))} aria-label="מחיקת מוצר"><Trash2 size={16} /></button> : null}</div>
      ...
      {!products.length ? <div className="empty"><b>עדיין לא הוגדר תמהיל לעובד</b><span>הוסיפו את המוצרים והאחוזים הקבועים כדי שהם ייכנסו אוטומטית לדיווחים חדשים.</span></div> : null}
    </div>
