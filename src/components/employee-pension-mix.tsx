...
import type { ContributionComponent, EmployeePensionProductInput, PensionProductType, SalaryAllocationType } from "@/lib/types";
...
export function EmployeePensionMix({ organizationId, employerId, employeeId, editable }: { organizationId: string; employerId: string; employeeId: string; editable: boolean }) {
  ...
  useEffect(() => {
    ...
    alphaApi.employeePensionMix(organizationId, employerId, employeeId)
      .then((items) => setProducts(items.map((item, index) => ({
        ...
        salaryAllocationValue: item.salaryAllocationValue == null ? (Number(item.salary) || null) : Number(item.salaryAllocationValue),
  ...
  async function save() {
    ...
    if (products.filter((product) => (product.salaryAllocationType ?? 1) === 4).length > 1) { setError("ניתן להגדיר מוצר אחד בלבד כיתרת שכר."); return; }
    if (products.some((product) => (product.salaryAllocationType ?? 1) !== 4 && Number(product.salaryAllocationValue ?? 0) <= 0)) { setError("יש להזין ערך חיובי לשכר קבוע, אחוז מהשכר או תקרה."); return; }
    if (products.some((product) => (product.salaryAllocationType ?? 1) === 2 && Number(product.salaryAllocationValue ?? 0) > 100)) { setError("אחוז מהשכר לא יכול להיות גבוה מ־100%."); return; }

    const normalized = products.map((product, index) => ({
      ...product,
      allocationOrder: Number.isFinite(Number(product.allocationOrder)) ? Number(product.allocationOrder) : index,
      salary: (product.salaryAllocationType ?? 1) === 1 ? Number(product.salaryAllocationValue ?? 0) : Number(product.salary || 0),
      salaryAllocationValue: (product.salaryAllocationType ?? 1) === 4 ? null : Number(product.salaryAllocationValue ?? 0),
    }));
  ...
  return <div className="card">
    <div className="card-head">
      <div><h2>תמהיל העובד</h2><span style={{ color: "var(--muted)" }}>כאן מגדירים איך שכר העובד מתחלק בין המוצרים. השכר המבוטח בפועל מחושב בכל דיווח לפי השכר החודשי.</span></div>
    ...
    <div className="product-editor-list">
      {products.map((product, index) => {
        ...
        const allocationType = (product.salaryAllocationType ?? 1) as SalaryAllocationType;
        return <section className="report-product-card" key={index} style={{ opacity: product.isActive ? 1 : 0.72 }}>
          ...
          <div className="grid report-product-fields">
            ...
            {allocationType !== 4 ? <div className="field"><label>{allocationValueLabel(allocationType)} *</label><input disabled={!editable} type="number" min="0" max={allocationType === 2 ? 100 : undefined} step={allocationType === 2 ? "0.01" : "0.01"} value={product.salaryAllocationValue ?? ""} onChange={(e) => updateProduct(index, { salaryAllocationValue: Number(e.target.value) })} /></div> : <div className="field"><label>ערך הקצאה</label><input disabled value="מחושב אוטומטית" /></div>}
            <div className="field"><label>סדר חישוב</label><input disabled={!editable} type="number" min="0" step="1" value={product.allocationOrder ?? index} onChange={(e) => updateProduct(index, { allocationOrder: Number(e.target.value) })} /></div>
            <div className="field"><label>גוף מוסדי</label><input disabled={!editable} maxLength={160} value={product.institutionalBody ?? ""} onChange={(e) => updateProduct(index, { institutionalBody: e.target.value })} /></div>
      ...
      {!products.length ? <div className="empty"><b>עדיין לא הוגדר תמהיל לעובד</b><span>הוסיפו מוצרים וחוקי הקצאת שכר כדי שהמערכת תחשב אותם אוטומטית בדיווח.</span></div> : null}
    </div>
