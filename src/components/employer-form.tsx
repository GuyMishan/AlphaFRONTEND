...
export function EmployerForm({ organizationId, employer, editable = true }: { organizationId: string; employer?: Employer; editable?: boolean }) {
  ...
  function update(key: keyof EmployerInput, value: string) { setForm((current) => ({ ...current, [key]: value })); setErrors((current) => ({ ...current, [key]: undefined })); }
  function validate(input: EmployerInput) {
    ...
    if (!/^\d{5,15}$/.test(input.withholdingFileNumber.trim())) next.withholdingFileNumber = "מספר תיק ניכויים חייב להכיל 5-15 ספרות.";
    return next;
  }
  ...
  return <div className="card profile-card"><div className="profile-summary"><div className="profile-avatar"><Building2 /></div><div><h2 style={{ margin: 0 }}>{title}</h2><span style={{ color: "var(--muted)" }}>{employer ? editable ? "עריכת פרופיל מעסיק" : "צפייה בפרופיל מעסיק" : "הקמת מעסיק חדש בארגון"}</span></div></div>{!editable && employer ? <div className="notice notice-info" style={{ marginBottom: 18 }}>יש לך הרשאת צפייה במעסיק הזה, ללא הרשאת עריכה.</div> : null}<form className="form" onSubmit={submit} noValidate><Field label="שם משפטי מלא *" error={errors.legalName}><input aria-invalid={Boolean(errors.legalName)} required minLength={2} maxLength={200} disabled={!editable} value={form.legalName} onChange={(event) => update("legalName", event.target.value)} /></Field><div className="grid two-cols"><Field label="מספר חברה / עוסק *" error={errors.registrationNumber}><input aria-invalid={Boolean(errors.registrationNumber)} required disabled={!editable} inputMode="numeric" maxLength={15} value={form.registrationNumber} onChange={(event) => update("registrationNumber", event.target.value.replace(/\D/g, "").slice(0, 15))} /></Field><Field label="מספר תיק ניכויים *" error={errors.withholdingFileNumber}><input aria-invalid={Boolean(errors.withholdingFileNumber)} required disabled={!editable} inputMode="numeric" maxLength={15} value={form.withholdingFileNumber} onChange={(event) => update("withholdingFileNumber", event.target.value.replace(/\D/g, "").slice(0, 15))} /></Field></div>{employer ? <div className="field"><label>סטטוס</label><input disabled value={employer.status === 2 ? "פעיל" : "בתהליך הקמה"} /></div> : null}<div className="form-actions"><Link className="btn btn-secondary" href="/employers">חזרה</Link>{editable ? <button className="btn btn-primary" disabled={saving} type="submit"><Save size={18} />{saving ? "שומר..." : employer ? "שמירת שינויים" : "הקמת מעסיק"}</button> : null}</div></form></div>;
}
