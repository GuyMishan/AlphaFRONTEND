...
export function InlineEmployeeCreateModal({ organizationId, employerId, onClose, onCreated, initial }: Props) {
  const [form, setForm] = useState<EmployeeInput>({
    ...
    postOfficeBox: initial?.postOfficeBox ?? "",
  });
  ...
  async function save() {
    ...
    if (!normalized.birthDate) errors.push("תאריך לידה הוא שדה חובה.");
    if (!normalized.gender) errors.push("מין העובד הוא שדה חובה.");
    ...
    if (!normalized.city) errors.push("יישוב הוא שדה חובה.");
    if (!normalized.street) errors.push("רחוב הוא שדה חובה.");
    if (!normalized.houseNumber) errors.push("מספר בית הוא שדה חובה.");
    if (!normalized.apartment) errors.push("מספר דירה הוא שדה חובה.");
    if (!/^\d+$/.test(normalized.postalCode)) errors.push("מיקוד הוא שדה חובה וחייב להכיל ספרות בלבד.");
    if (!normalized.postOfficeBox) errors.push("תא דואר הוא שדה חובה.");
    if (errors.length) {
    ...
    <div className="report-modal" role="dialog" aria-modal="true" aria-label="הקמת עובד חדש">
      ...
      <div className="report-modal-body">
        ...
        <div className="grid two-cols">
          ...
          <div className="field"><label>יישוב *</label><input maxLength={100} value={form.city} onChange={(e) => update("city", e.target.value)} /></div>
          <div className="field"><label>רחוב *</label><input maxLength={100} value={form.street} onChange={(e) => update("street", e.target.value)} /></div>
          <div className="field"><label>מספר בית *</label><input maxLength={20} value={form.houseNumber} onChange={(e) => update("houseNumber", e.target.value)} /></div>
          <div className="field"><label>מספר דירה *</label><input maxLength={20} value={form.apartment} onChange={(e) => update("apartment", e.target.value)} /></div>
          <div className="field"><label>מיקוד *</label><input inputMode="numeric" maxLength={10} value={form.postalCode} onChange={(e) => update("postalCode", e.target.value.replace(/\D/g, "").slice(0, 10))} /></div>
          <div className="field"><label>תא דואר *</label><input maxLength={20} value={form.postOfficeBox} onChange={(e) => update("postOfficeBox", e.target.value)} /></div>
        </div>
      </div>
