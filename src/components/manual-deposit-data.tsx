...
export function ManualDepositData({ organizationId, employerId, reportId }: { organizationId: string; employerId: string; reportId: string }) {
  ...
  return <>
    ...
    <div className="toolbar deposit-toolbar"><div className="search"><Search size={17} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="יצרן, מוצר, עובד או אסמכתא" /></div></div>
    ...
    <div className="payment-modal" role="dialog" aria-modal="true" aria-label="פרטי אמצעי תשלום">
      ...
      <div className="payment-layout">
        ...
        <div className="payment-main">
          <section className="payment-panel"><h3>פרטי חשבון יצרן לזיכוי</h3><div className="payment-provider-grid"><div className="field payment-provider-name"><label>שם יצרן / מוצר</label><input value={form.providerName} onChange={(e) => patch("providerName", e.target.value)} /></div><div className="payment-amount"><span>סכום</span><b>₪{Number(row.totalDeposit).toLocaleString("he-IL")}</b></div><div className="field payment-provider-account"><label>חשבון יצרן לזיכוי</label><div className="payment-input-icon"><input value={form.providerAccount} onChange={(e) => patch("providerAccount", e.target.value)} placeholder="בנק - סניף - חשבון" /><Pencil size={16} /></div></div></div></section>
          <section className="payment-panel"><h3>פרטי אופן תשלום</h3><div className="payment-method-grid"><div className="field payment-method"><label>אופן התשלום</label><select value={form.paymentMethod} onChange={(e) => patch("paymentMethod", e.target.value)}><option>העברה בנקאית</option><option>מס״ב</option><option>המחאה</option><option>אחר</option></select></div><div className="field"><label>תאריך ערך</label><div className="payment-input-icon"><input type="date" value={form.valueDate?.slice(0, 10) || ""} onChange={(e) => patch("valueDate", e.target.value || null)} /><CalendarDays size={16} /></div></div><div className="field"><label>מס׳ אסמכתא</label><input value={form.referenceNumber} onChange={(e) => patch("referenceNumber", e.target.value)} /></div><label className="payment-upload"><FileUp size={17} /><span>{form.confirmationFileName || "צירוף אישור העברה"}</span><input type="file" hidden onChange={(e) => patch("confirmationFileName", e.target.files?.[0]?.name || "")} /></label><div className="field"><label>בנק</label><input value={form.employerBankName} onChange={(e) => patch("employerBankName", e.target.value)} placeholder="שם הבנק" /></div><div className="field"><label>מס׳ בנק</label><input inputMode="numeric" value={form.employerBankCode} onChange={(e) => patch("employerBankCode", e.target.value)} /></div><div className="field"><label>סניף</label><input inputMode="numeric" value={form.employerBranch} onChange={(e) => patch("employerBranch", e.target.value)} /></div><div className="field"><label>מס׳ חשבון</label><input inputMode="numeric" value={form.employerAccount} onChange={(e) => patch("employerAccount", e.target.value)} /></div></div></section>
        </div>
      </div>
