  ...
  return <section className="card profile-card">
    ...
    <form className="form" onSubmit={save}>
      <div className="grid two-cols">
        <div className="field"><label>שם הארגון</label><input disabled={!profile.canManageOrganization} required maxLength={200} value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></div>
        <div className="field"><label>סוג ארגון</label><select disabled={!profile.canManageOrganization} value={form.type} onChange={(e) => setForm({ ...form, type: Number(e.target.value) })}><option value={1}>מעסיק</option><option value={2}>משרד שכר</option><option value={3}>סוכנות ביטוח</option><option value={4}>ספק תפעול</option><option value={5}>קבוצת חברות</option></select></div>
        <div className="field"><label>מספר חברה / עוסק</label><input disabled={!profile.canManageOrganization} maxLength={30} value={form.registrationNumber} onChange={(e) => setForm({ ...form, registrationNumber: e.target.value })} /></div>
        <div className="field"><label>איש קשר</label><input disabled={!profile.canManageOrganization} maxLength={150} value={form.contactName} onChange={(e) => setForm({ ...form, contactName: e.target.value })} /></div>
        <div className="field"><label>אימייל איש קשר</label><input disabled={!profile.canManageOrganization} type="email" maxLength={320} value={form.contactEmail} onChange={(e) => setForm({ ...form, contactEmail: e.target.value })} /></div>
        <div className="field"><label>טלפון איש קשר</label><input disabled={!profile.canManageOrganization} maxLength={20} value={form.contactPhone} onChange={(e) => setForm({ ...form, contactPhone: e.target.value.replace(/\D/g, "") })} /></div>
        <div className="field"><label>יישוב</label><input disabled={!profile.canManageOrganization} maxLength={100} value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} /></div>
        <div className="field"><label>רחוב</label><input disabled={!profile.canManageOrganization} maxLength={100} value={form.street} onChange={(e) => setForm({ ...form, street: e.target.value })} /></div>
        <div className="field"><label>מספר בית</label><input disabled={!profile.canManageOrganization} maxLength={20} value={form.houseNumber} onChange={(e) => setForm({ ...form, houseNumber: e.target.value })} /></div>
        <div className="field"><label>דירה</label><input disabled={!profile.canManageOrganization} maxLength={20} value={form.apartment} onChange={(e) => setForm({ ...form, apartment: e.target.value })} /></div>
        <div className="field"><label>מיקוד</label><input disabled={!profile.canManageOrganization} maxLength={10} value={form.postalCode} onChange={(e) => setForm({ ...form, postalCode: e.target.value.replace(/\D/g, "") })} /></div>
        <div className="field"><label>תא דואר</label><input disabled={!profile.canManageOrganization} maxLength={20} value={form.postOfficeBox} onChange={(e) => setForm({ ...form, postOfficeBox: e.target.value })} /></div>
      </div>
