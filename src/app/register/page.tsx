...
export default function RegisterPage() {
  ...
  async function requestCode() {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 409) toast.error("כבר קיים משתמש עם האימייל, תעודת הזהות או מספר הטלפון האלה.");
  ...
  async function verify(event: FormEvent) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        else if (response.status === 409) toast.error("משתמש עם הפרטים האלה כבר נוצר.");
