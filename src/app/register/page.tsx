...
export default function RegisterPage() {
  ...
  async function requestCode() {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 409) toast.error("כבר קיים משתמש עם תעודת הזהות או מספר הפלאפון האלה. כרגע לא ניתן ליצור משתמש נוסף עם אותם פרטים; נטפל בתרחיש הזה בהמשך.");
  ...
  async function verifyCode(codeToVerify = code) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        else if (response.status === 409) toast.error("כבר קיים משתמש עם תעודת הזהות או מספר הפלאפון האלה. כרגע לא ניתן ליצור משתמש נוסף עם אותם פרטים; נטפל בתרחיש הזה בהמשך.");
