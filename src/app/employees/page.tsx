...
export default function EmployeesPage() {
  ...
  return <AppShell title="עובדים"><div className="page-head"><div><h1>עובדים</h1><p>{employer ? `עובדי ${employer.legalName}` : "בחרו מעסיק בדף הבית"}</p></div><button className="btn btn-primary"><UserRoundPlus size={18} />הוספת עובד</button></div>{error ? <div className="notice notice-error">{error}</div> : null}<section className="card"><div className="toolbar"><div className="search"><Search size={17} /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="חיפוש עובד" /></div><span className="badge badge-blue">{employees.length} עובדים</span></div>{loading ? <div className="empty">טוען עובדים מה־Backend...</div> : filtered.length ? <div className="table-wrap"><table><thead><tr><th>שם</th><th>תעודת זהות</th><th>מספר עובד</th><th>תאריך התחלה</th><th>תאריך סיום</th><th>סטטוס</th></tr></thead><tbody>{filtered.map((employee) => <tr key={employee.id}><td><b>{employee.firstName} {employee.lastName}</b></td><td>{employee.nationalId}</td><td>{employee.employeeNumber}</td><td>{employee.startDate}</td><td>{employee.endDate ?? "—"}</td><td><span className={employee.status === 1 ? "badge badge-green" : "badge badge-gray"}>{employee.status === 1 ? "פעיל" : employee.status === 2 ? "חל״ת" : "סיים עבודה"}</span></td></tr>)}</tbody></table></div> : <div className="empty"><Users size={35} /><div>לא נמצאו עובדים למעסיק הזה.</div></div>}</section></AppShell>;
}
