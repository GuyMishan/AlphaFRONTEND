  ...
  async function requestCode() {
    ...
    try {
      const response = await fetch("/api/backend/api/auth/otp/request", {
        ...
        body: JSON.stringify({ nationalId, phone, channel: "email" }),
        cache: "no-store",
      });
      ...
      if (!response.ok) {
        toast.error(
          response.status === 401
            ? "תעודת הזהות או מספר הטלפון אינם תואמים"
            : response.status === 429
              ? "ניתן לבקש קוד חדש בעוד דקה."
              : "לא ניתן לשלוח קוד כרגע. נסו שוב מאוחר יותר.",
        );
        return;
      }
  ...
  async function verifyCode(codeToVerify = code) {
    ...
    try {
      ...
      if (!response.ok) {
        toast.error(
          response.status === 401
            ? "הקוד שגוי, פג תוקף או נוצל. נסו שוב או בקשו קוד חדש."
            : "לא ניתן להתחבר כרגע.",
        );
        return;
      }
    ...
    <main className="auth-page">
      ...
      <section className="auth-form-wrap">
        <div className="auth-card">
          <h2>כניסה למערכת</h2>
          <p>{challenge ? "הזינו את הקוד בן 6 הספרות שנשלח לדוא״ל הרשום" : "התחברו כדי להמשיך לסביבת העבודה שלכם"}</p>
          ...
          {!challenge ? (
            <form className="form" onSubmit={(event) => { event.preventDefault(); void requestCode(); }} noValidate>
              ...
              <button className="btn btn-primary btn-lg wide" disabled={loading} type="submit">
                {loading ? "שולח..." : "שלחו לי קוד בדוא״ל"}
                <ArrowLeft size={18} />
              </button>
            </form>
          ) : (
            <form className="form" onSubmit={verify} noValidate>
              ...
              <button className="btn wide" disabled={loading} type="button" onClick={() => void requestCode()}>
                שלחו קוד חדש
              </button>
              <button className="btn wide" disabled={loading} type="button" onClick={() => { setChallenge(null); setCode(""); }}>
                שינוי פרטי כניסה
              </button>
            </form>
          )}
