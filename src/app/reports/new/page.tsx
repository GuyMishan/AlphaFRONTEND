...
const excelSteps = ["פרטי הדיווח", "העלאת קובץ שכר", "נתוני הפקדות", "סיכום ושליחה"];
...
export default function NewReportPage() {
  ...
  async function loadScope(nextScope: { organizationId: string; employerId: string }) {
    setLoading(true); setError(""); setScope(nextScope); setManualReportId(""); setSelectedSourceReportId(""); setSourceReports([]);
    try {
  ...
  useEffect(() => {
    ...
    setMode("manual"); setManualReportId(""); setSourceSearch(""); void loadSources(true, "");
  }, [reportKind, scope?.organizationId, scope?.employerId]);

  function chooseSource(report: SourceManualReport) {
    setSelectedSourceReportId(report.id); setMonth(report.reportingMonth.slice(0, 7)); setSalaryPaymentDate(report.salaryPaymentDate?.slice(0, 10) ?? ""); setError("");
  }
  function chooseKind(kind: ManualReportKind) { setReportKind(kind); setManualReportId(""); setSelectedSourceReportId(""); setFileName(""); setError(""); if (kind !== 1) setMode("manual"); }
  const canContinue = step === 1 ? Boolean(employer && month && salaryPaymentDate && (reportKind === 1 || selectedSourceReportId)) : step === 2 ? reportKind === 1 && mode === "excel" ? Boolean(fileName) : selectedIds.length > 0 : true;

  function selectFile(event: ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0];
    if (!file) { setFileName(""); return; }
    if (!/\.(xlsx|xls|csv)$/i.test(file.name)) { setFileName(""); setError("יש לבחור קובץ Excel או CSV בלבד."); return; }
    if (file.size > 20 * 1024 * 1024) { setFileName(""); setError("הקובץ גדול מדי. הגודל המרבי הוא 20MB."); return; }
  ...
  async function next() {
    ...
    try {
      ...
      if (step === 2) {
        if (reportKind === 1 && mode === "excel") {
          if (!fileName) throw new Error("יש לבחור קובץ תקין לפני שממשיכים.");
      ...
      if (step === 3 && manualReportId && !(reportKind === 1 && mode === "excel")) await validatePersistedReport("deposits");
