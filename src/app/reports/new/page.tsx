    ...
    try {
      if (step === 1 && mode === "manual") {
        if (!manualReportId) {
          ...
          await Promise.all([
            alphaApi.updateManualReportDetails(scope.organizationId, scope.employerId, manualReportId, {
              reportingMonth: `${month}-01`, salaryPaymentDate: salaryPaymentDate || null,
            }),
      ...
      <div className="wizard">
        ...
        {loading ? <div className="card empty">טוען את המעסיק והעובדים מה־Backend...</div> : !employer ? <div className="card empty"><Info size={34} /><h3>עדיין לא נבחר מעסיק</h3><button className="btn btn-primary" onClick={() => router.push("/dashboard")}>לבחירת מעסיק</button></div> : (
          <section className="card">
            ...
            {step === 3 ? <Validation mode={mode} selectedCount={selectedIds.length} fileName={fileName} reportId={manualReportId} /> : null}
            {step === 4 ? <Summary employer={employer} month={month} mode={mode} selectedCount={selectedIds.length} fileName={fileName} reportId={manualReportId} /> : null}
            <div className="wizard-footer">
              <button className="btn btn-secondary" disabled={step === 1 || advancing} onClick={() => setStep((value) => value - 1)}><ArrowRight size={17} />חזרה</button>
              {step < 4 ? <button className="btn btn-primary" disabled={!canContinue || advancing} onClick={() => void next()}>{advancing ? "שומר..." : "המשך"}<ArrowLeft size={17} /></button> : <button className="btn btn-primary" disabled title="שליחת הדיווח לגופים תתווסף בשלב הבא"><Send size={17} />שליחת דיווח</button>}
            </div>
          </section>
        )}
      </div>
...
function ReportDetails({ mode, setMode, month, setMonth, salaryPaymentDate, setSalaryPaymentDate }: {
  mode: ReportMode;
...
function ExcelData({ fileName, selectFile }: { fileName: string; selectFile: (event: ChangeEvent<HTMLInputElement>) => void }) {
  return <><div className="card-head"><div><h2>העלאת קובץ שכר</h2><span style={{ color: "var(--muted)" }}>קובצי Excel בלבד, עד 10MB</span></div></div><label className="upload-zone" htmlFor="excel-file"><UploadCloud size={38} /><h3>{fileName || "גררו קובץ לכאן או בחרו מהמחשב"}</h3><p>הקובץ ייבדק לפני המשך התהליך</p><span className="btn btn-soft">בחירת קובץ</span><input id="excel-file" type="file" accept=".xlsx,.xls,.csv" hidden onChange={selectFile} /></label><div className="notice notice-warning" style={{ marginTop: 18 }}>קליטת Excel אמיתית תתחבר ל־Backend בשלב הבא.</div></>;
}

function CorrectionData({ reason, setReason }: { reason: string; setReason: (value: string) => void }) {
  return <><div className="card-head"><div><h2>פרטי התיקון</h2><span style={{ color: "var(--muted)" }}>בחירת דיווח מקור תתווסף עם מודול היסטוריית הדיווחים</span></div></div><div className="grid two-cols"><div className="field"><label>סוג התיקון</label><select><option>תיקון סכומים</option><option>עובד שנשכח</option><option>עובד שדווח בטעות</option><option>תיקון מוצר פנסיוני</option><option>דיווח הפרשים</option></select></div><div className="field"><label>דיווח מקור</label><select disabled><option>ייבחר בהמשך</option></select></div></div><div className="field" style={{ marginTop: 18 }}><label htmlFor="reason">סיבת התיקון</label><textarea id="reason" value={reason} onChange={(e) => setReason(e.target.value)} placeholder="פרטו מה נדרש לתקן ולמה" /></div></>;
}

function Validation({ mode, selectedCount, fileName, reportId }: { mode: ReportMode; selectedCount: number; fileName: string; reportId: string }) {
  return <><div className="card-head"><div><h2>בדיקות ובקרות</h2><span style={{ color: "var(--muted)" }}>סיכום הנתונים שנאספו בשלב הקודם</span></div></div><div className="grid two-cols"><div className="notice notice-info"><Check size={17} /> המעסיק וההרשאות אומתו מול ה־Backend</div><div className="notice notice-info"><Check size={17} /> {mode === "manual" ? `${selectedCount} עובדים נשמרו בטיוטה${reportId ? "" : ""}` : fileName || "פרטי התיקון נקלטו"}</div></div><div className="notice notice-warning" style={{ marginTop: 18 }}>בשלב הבא נוסיף Validation מלא: מוצרים חסרים, סכומים, אחוזים, כפילויות ותקינות נתוני ההפקדה.</div></>;
}

function Summary({ employer, month, mode, selectedCount, fileName, reportId }: { employer: Employer; month: string; mode: ReportMode; selectedCount: number; fileName: string; reportId: string }) {
  const modeLabel = mode === "manual" ? "דיווח ידני" : mode === "excel" ? "קובץ Excel" : "תיקון דיווח";
  return <><div className="card-head"><div><h2>סיכום הדיווח</h2><span style={{ color: "var(--muted)" }}>הדיווח עדיין בטיוטה ולא נשלח לגופים.</span></div></div><div className="summary-row"><b>מעסיק</b><span>{employer.legalName}</span></div><div className="summary-row"><b>חודש דיווח</b><span>{month}</span></div><div className="summary-row"><b>אופן דיווח</b><span>{modeLabel}</span></div><div className="summary-row"><b>{mode === "manual" ? "עובדים בדיווח" : "קובץ"}</b><span>{mode === "manual" ? selectedCount : fileName || "—"}</span></div>{reportId ? <div className="notice notice-info" style={{ marginTop: 18 }}>טיוטת הדיווח נשמרה ב־Backend וב־Supabase.</div> : null}</>;
}
