...
const PAGE_SIZE = 50;
const roleNames: Record<OrganizationRole, string> = { 1: "מנהל ארגון", 2: "מנהל שכר", 3: "נציג תפעול", 4: "צפייה בלבד" };
...
export default function AccessPage() {
  ...
  return <AppShell title="משתמשים והרשאות">
    ...
    {adding ? <section className="card" style={{ marginBottom: 18 }}><h2 style={{ marginTop: 0 }}>הוספת משתמש לארגון</h2><div className="toolbar"><div className="search"><Search size={17} /><input value={candidateSearch} onChange={(e) => setCandidateSearch(e.target.value)} placeholder="לפחות 2 תווים בשם או באימייל" /></div></div>{candidates.length ? <VirtualizedTable items={candidates} maxHeight={260} rowHeight={52} rowKey={(item) => item.id} onRowClick={setCandidate} columns={[{ key: "name", label: "משתמש" }, { key: "email", label: "אימייל" }, { key: "selected", label: "" }]} renderCells={(item) => [<b>{item.displayName}</b>, item.email, candidate?.id === item.id ? <span className="badge badge-blue">נבחר</span> : null]} /> : null}{candidate ? <div className="grid two-cols" style={{ marginTop: 18 }}><div className="field"><label>תפקיד</label><select value={newRole} onChange={(e) => setNewRole(Number(e.target.value) as OrganizationRole)}>{Object.entries(roleNames).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div><div className="field"><label>גישה למעסיקים</label><select value={newAccessMode} onChange={(e) => setNewAccessMode(Number(e.target.value) as EmployerAccessMode)}><option value={1}>כל המעסיקים בארגון</option><option value={2}>מעסיקים מסוימים בלבד</option></select></div><div className="form-actions"><button className="btn btn-primary" onClick={addUser} type="button"><Plus size={18} />הוספה לארגון</button></div></div> : null}</section> : null}

    <section className="card" style={{ marginBottom: 18 }}><div className="toolbar"><div className="search"><Search size={17} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="חיפוש משתמש לפי שם או אימייל" /></div><span className="badge badge-blue">{organizations.find((x) => x.id === organizationId)?.name ?? "ארגון"}</span></div>{loading ? <div className="empty">טוען משתמשים...</div> : users.length ? <VirtualizedTable items={users} rowKey={(item) => item.userId} onRowClick={setSelected} columns={[{ key: "user", label: "משתמש" }, { key: "email", label: "אימייל" }, { key: "role", label: "תפקיד" }, { key: "access", label: "גישה למעסיקים" }]} renderCells={(item) => [<b>{item.displayName}</b>, item.email, roleNames[item.role], item.employerAccessMode === 1 ? "כל המעסיקים" : "מעסיקים מסוימים"]} /> : <div className="empty"><ShieldCheck size={34} /><div>לא נמצאו משתמשים.</div></div>}<div className="form-actions" style={{ marginTop: 14 }}><button className="btn btn-secondary" disabled={skip === 0} onClick={() => setSkip(Math.max(0, skip - PAGE_SIZE))}>הקודם</button><button className="btn btn-secondary" disabled={!hasMore} onClick={() => setSkip(skip + PAGE_SIZE)}>הבא</button></div></section>

    {selected ? <section className="card"><div className="page-head" style={{ marginBottom: 12 }}><div><h2 style={{ margin: 0 }}>{selected.displayName}</h2><p>{selected.email}</p></div><button className="btn btn-secondary" onClick={removeUser}><Trash2 size={17} />הסרת גישה</button></div><div className="grid two-cols"><div className="field"><label>תפקיד בארגון</label><select value={role} onChange={(e) => setRole(Number(e.target.value) as OrganizationRole)}>{Object.entries(roleNames).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div><div className="field"><label>גישה למעסיקים</label><select value={accessMode} onChange={(e) => setAccessMode(Number(e.target.value) as EmployerAccessMode)}><option value={1}>כל המעסיקים בארגון</option><option value={2}>מעסיקים מסוימים בלבד</option></select></div></div><div className="grid two-cols" style={{ marginTop: 16 }}><div className="notice notice-info"><b>הרשאות בפועל</b><div style={{ marginTop: 8 }}>הקמת מעסיק: {capabilities.createEmployer ? "כן" : "לא"}</div><div>עריכת מעסיק: {capabilities.editEmployer ? "כן" : "לא"}</div><div>הקמת עובד: {capabilities.createEmployee ? "כן" : "לא"}</div><div>עריכת עובד: {capabilities.editEmployee ? "כן" : "לא"}</div>{accessMode === 2 && role !== 4 ? <div style={{ marginTop: 6 }}>הפעולות מותרות רק במעסיקים שהוקצו למשתמש.</div> : null}</div><div className="form-actions" style={{ alignItems: "flex-end" }}><button className="btn btn-primary" disabled={saving} onClick={saveAccess}><Save size={18} />{saving ? "שומר..." : "שמירת הרשאות"}</button></div></div>
