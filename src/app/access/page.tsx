...
export default function AccessPage() {
  ...
  return <AppShell title="משתמשים והרשאות">
    ...
    {inviteOpen ? <section className="card" style={{ marginBottom: 18 }}>
      ...
      <div className="field"><label>אימייל</label><input type="email" dir="ltr" maxLength={320} value={inviteEmail} onChange={(e) => setInviteEmail(e.target.value)} placeholder="name@example.com" /></div>
      ...
      {inviteScope === "organization" ? <div className="field" style={{ marginTop: 16 }}><label>תפקיד בארגון</label><ReferenceOptionSelect category="organization-role" value={inviteOrganizationRole} onChange={(value) => setInviteOrganizationRole(Number(value) as OrganizationRole)} /></div> :
        <div className="grid two-cols" style={{ marginTop: 16 }}>
          <div className="field"><label>מעסיק</label><select value={inviteEmployerId} onChange={(e) => setInviteEmployerId(e.target.value)}>{inviteEmployers.map((item) => <option key={item.id} value={item.id}>{item.legalName}</option>)}</select></div>
          <div className="field"><label>תפקיד אצל המעסיק</label><select value={inviteEmployerRole} onChange={(e) => setInviteEmployerRole(Number(e.target.value) as EmployerRole)}>{(Object.keys(employerRoleLabels) as unknown as EmployerRole[]).map((value) => <option key={value} value={value}>{employerRoleLabels[value]}</option>)}</select></div>
        </div>}
