...
export default function NewEmployerPage() {
  ...
  return <AppShell title="הקמת מעסיק">
    ...
    {!scopeResolved || loading ? <div className="empty">טוען הרשאות...</div>
      ...
      : entitlements ? <><section className="card" style={{ marginBottom: 18 }}><div className="card-head" style={{ marginBottom: 10 }}><div><h3>שימוש במסלול {entitlements.plan.name}</h3></div></div><PlanUsage label="מעסיקים" usage={entitlements.employers} /></section>{entitlements.employers.maximum !== null && entitlements.employers.current >= entitlements.employers.maximum ? <div className="notice notice-info"><b>הגעתם למגבלת המעסיקים במסלול.</b><div style={{ marginTop: 10 }}><button className="btn btn-primary" type="button" onClick={() => openUpgradeDialog({ reason: "employers", planName: entitlements.plan.name, current: entitlements.employers.current, maximum: entitlements.employers.maximum ?? undefined })}>יצירת קשר לשדרוג</button></div></div> : <EmployerForm organizationId={organizationId} />}</>
