...
export default function EmployerProfilePage() {
  ...
  useEffect(() => { if (organizationId && id) alphaApi.employer(organizationId, id).then(setEmployer).catch((err) => setError(err instanceof Error ? err.message : "טעינת המעסיק נכשלה")); }, [id, organizationId]);
  return <AppShell title="פרופיל מעסיק" hideScopeController><div className="page-head"><div><h1>פרופיל מעסיק</h1><p>פרטים משפטיים והגדרות זיהוי של המעסיק.</p></div></div>{error ? <div className="notice notice-error">{error}</div> : employer ? <EmployerForm organizationId={organizationId} employer={employer} /> : <div className="empty">טוען פרופיל...</div>}</AppShell>;
}
